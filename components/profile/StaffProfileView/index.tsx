...
interface StaffDossierSectionProps {
...
function StaffDossierSection({ profile, email, accentColor, accentBg }: StaffDossierSectionProps) {
      ...
      <form onSubmit={handleSaveDossier} className="bg-white p-6 rounded-2xl shadow-ambient border border-[var(--bg-border)] flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col gap-3">
            <div>
              <label htmlFor="staff-display-name" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
                Display Name
              </label>
              <input
            ...
            <div>
              <label htmlFor="staff-preferred-role" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
                Preferred Focus / Role
              </label>
              <input
                id="staff-preferred-role"
                type="text"
                value={preferredRole}
            ...
            <div>
              <label htmlFor="staff-primary-area" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
                Primary Area
              </label>
              <input
                id="staff-primary-area"
                type="text"
            ...
            <div>
              <label htmlFor="staff-emergency-phone" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
          ...
          <div className="flex flex-col gap-3">
            <div>
              <label htmlFor="staff-avatar-url" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
                Avatar Image URL
              </label>
              <input
                id="staff-avatar-url"
            ...
            <div>
              <label htmlFor="staff-avatar-upload" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
                OR Upload Photo
              </label>
              <input
            ...
            <div className="flex-grow flex flex-col">
              <label htmlFor="staff-bio" className="block font-body text-[10px] font-bold text-[var(--empire-cream)]/50 uppercase tracking-wider mb-1">
...
interface VolunteerTabSectionProps {
  readonly profile: StaffProfile;
  readonly email: string;
  readonly sightings: Sighting[];
  readonly pledges: Pledge[];
  readonly recentPoints: Array<{ activity: string; points: number; created_at: string }>;
  readonly badges: Array<{ badge_id: string; earned_at: string }>;
}

function VolunteerTabSection({ profile, email, sightings, pledges, recentPoints, badges }: VolunteerTabSectionProps) {
  return (
...
export default function StaffProfileView({
  ...
  if (daysRemaining !== null) {
    if (daysRemaining === 0) {
      trialBannerText = 'Trial Access Expired — Contact admin to renew';
    } else if (daysRemaining === 1) {
      trialBannerText = 'Trial Access: 1 day remaining';
    } else {
      trialBannerText = `Trial Access: ${daysRemaining} days remaining`;
    }
  }

  let subModBannerText = '';
  if (isSubMod) {
    if (editsCount >= maxEdits) {
      subModBannerText = 'Edit limit reached — further edits will be rejected at the database level';
    } else if (maxEdits - editsCount === 1) {
      subModBannerText = '1 edit remaining before limit is enforced';
    } else {
      subModBannerText = `${maxEdits - editsCount} edits remaining before limit is enforced`;
    }
  }
    ...
    <div className="w-full max-w-7xl mx-auto px-4 md:px-12 py-8 flex flex-col gap-8">
        ...
        <div className={`flex items-center gap-3 px-5 py-3 rounded-2xl border ${subModBannerClass}`}>
          ...
          <div className="flex-grow flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="w-24 h-1.5 bg-[var(--bg-border)]/30 rounded-full overflow-hidden flex-shrink-0">
              <div className="h-full rounded-full" style={{ width: `${Math.min((editsCount / maxEdits) * 100, 100)}%`, backgroundColor: progressBarColor }} />
            </div>
          </div>
        </div>
      )}

      {/* Segmented Slider Tab Selector */}
      <div className="flex border-b border-[var(--bg-border)]/20 pb-0.5">
        {[
          { id: 'staff', label: 'Staff Profile' },
          { id: 'volunteer', label: 'Volunteer Profile' },
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() => setProfileTab(tab.id as 'staff' | 'volunteer')}
            className={`py-3.5 px-6 font-display text-sm font-extrabold capitalize border-b-2 transition-all cursor-pointer ${
              profileTab === tab.id
      ...
      {profileTab === 'staff' ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          {/* Dossier (8 columns) */}
          <div className="lg:col-span-8 flex flex-col gap-6 w-full">
            <StaffDossierSection profile={profile} email={email} accentColor={accentColor} accentBg={accentBg} />
          </div>

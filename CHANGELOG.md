...
### Fixed
- **Theme-Compliant Modals & Inputs**: Cleaned up form inputs and textareas in `app/globals.css` to be global across both themes, rather than nested only inside the light mode wrapper block. Updated `SupplyRequestModal` and `SkillsPanel` modals to dynamically inherit theme colors (`bg-[var(--bg-surface)]`, `border-[var(--bg-border)]`, and `text-[var(--text-primary)]`) and forms to use inputs (`bg-[var(--input-bg)]`, `border-[var(--input-border)]`) with global focus rings and premium buttons (`btn btn-ghost`, `btn btn-teal`), resolving accessibility contrast issues in light theme and mismatch states.
- **Volunteer Skill Credentials Verification System**: Added database columns and Server Actions (`claimSkill` with info/proof, `verifySkill`, `rejectSkill`, `raiseQueryOnSkill`, `respondToSkillQuery`, and `getPendingSkills`) supporting a multi-phase verification workflow. Refactored `SkillsPanel` to show user-facing submission details, query prompts, response fields, and a global Moderator Verification Queue.
...
- **Onboarding Emoji Cleanup**: Replaced emoji characters in `OnboardingClient.tsx` with Material Symbols icons to adhere to the code-quality non-emoji constraint.
- **Onboarding Header Visual Cleanup**: Removed the bouncing cat/pets logo accent floating behind the MeowNet header logo to resolve the visual clutter.
- **Role Card Ligature Overflow**: Added `overflow-hidden` to role card icons and changed the TNR Expert icon from `catching_pokemon` (unsupported in the loaded font) to `pets` (paw print), preventing text ligatures from overflowing and displaying as `G_` when fonts fallback.
- **Cat Deletion Auto-Audit Constraint**: Created migration `20260705173420_fix_audit_triggers.sql` to redefine trigger functions (`trigger_audit_cats`, `trigger_audit_tnr_events`, and `trigger_audit_moderator_queries`) to check if the `v_actor_id` exists in `public.profiles` and fallback to `'00000000-0000-0000-0000-000000000000'::UUID` (System Cats admin profile), preventing database foreign key constraint violations on `staff_audit_logs.actor_id` when deleting cat sightings belonging to volunteers without a profile record.
- **ML Classification Fallback**: Added graceful local fallbacks (returning `"Domestic Shorthair"` for breed and `"Happy"` for mood) to HuggingFace Async Client classifiers in `python-ml/infrastructure/hf_adapter.py` to prevent 500 Internal Server Error when external Hugging Face API queries fail, and supported both `HF_API_TOKEN` and `HUGGINGFACE_API_KEY` env vars.
- **ML Breed Classification Cold-Start Fix**: Implemented a retry loop with exponential backoff and specified the `"x-wait-for-model": "true"` header. This instructs the Hugging Face API server to hold the request until the model is loaded rather than returning a 503 error, ensuring the application receives actual breed results.
- **DOM XSS Sanitization**: Wrapped volunteer-supplied cat image URLs with `getSafeImageSrc` in `profile/page.tsx`, `profile/[id]/page.tsx`, and `reports/page.tsx` to meet DOM XSS security constraints.
- **AI Consent Gate UI Lock**: Disabled the "Yes, use AI" button in `components/forms/ConsentGate/index.tsx` to lock the option, displaying a lock icon with the text "Coming soon". Styled the manual entry button as the primary action option to guide users to manual details entry.
- **Cat Sighting Photo Upload UI Cleanup**: Replaced the persistent dashed file uploader area in `LogCatForm/index.tsx` with a conditional render that swaps out the upload container entirely for the cat photo preview upon a successful image selection. Added a styled close/delete button to the top-right corner of the preview to let users discard the photo and return to the upload interface.
- **Null Safety in Cat Sighting DB Audits**: Created migration `20260705175800_fix_audit_cats_null_name.sql` to handle null/unnamed cat profiles gracefully in `trigger_audit_cats` with `COALESCE(NEW.name, 'Unnamed')`, preventing string concatenation crashes that triggered `insert_failed` errors on cat creation. Wrapped trigger execution blocks in structured exception handlers to isolate audit log insertion failures and prevent them from blocking the user's primary cat logging transaction.
- **Light Theme Input Accessibility**: Styled inputs, selects, and textareas in `LogCatForm/index.tsx` with `bg-[var(--input-bg)]` and updated grid layout configurations to prevent text and placeholders from blending into light backgrounds or getting squished and cut off on tablet viewports.
- **Breed Confidence Preprocessing**: Integrated Zod preprocessing for `breed_confidence` in `cats.ts` to convert empty form values to null before validation checks.
- **Cat Sighting Details Input and Information Enhancements**: Updated placeholders for Cat Name, Primary Color / Pattern, and Breed Estimate to be shorter (`e.g. Whiskers`, `e.g. Orange tabby`, and `e.g. Tabby` respectively) to prevent layout cuts on smaller screen sizes. Added frontend field validation to check if the required Primary Color / Pattern field is empty, returning a precise field-specific validation message to guide user input. Upgraded the Step 1 information block to a structured card matching key styling patterns.
- **Log Cat Form Step 3 Visual Alignment & Enhancements**: Fixed alignment imbalances in Step 3 inputs by setting standard base height limits (`min-h-[40px]`) on label spans. Added a disabled **Use AI** button with a lock icon and "coming soon" indicator inline with the header, accompanied by a hover tooltip showing `"Use photo to identify breed & health indicators automatically"`. Redesigned **Community Care Indicators** from generic checkboxes into styled interactive pills with hover transitions. Hardened **Triage & Health Indicators** for dark mode with theme-compliant colors (`bg-red-500/10 border-red-500/40 text-red-600 dark:text-red-400`).
- **ReDoS Vulnerability in Text Sanitization**: Refactored the `sanitizeText` utility in `lib/security/sanitize.ts` to replace the polynomial regular expression `/<[^>]*>/g` with a linear-time O(N) tag-stripping scan, eliminating the potential for Regular Expression Denial of Service (ReDoS) on user-supplied input.

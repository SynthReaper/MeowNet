...
Retrieves the list of educational academy courses.

**Security:**
- Filters out unpublished courses for non-staff callers.
- Strips the `correct_answer` field from all quiz questions inside the response payload for non-staff callers, preventing trivia leakage.

---

### `GET /api/emergency/incidents`

Retrieves the list of emergency dispatcher incidents.

**Security:**
- Strips staff-only internal tracking fields (e.g., `dispatch_notes`, `internal_status`) from the response unless the caller is authenticated as staff (admin/moderator).

---

### `GET /api/chapters/members`

Retrieves the chapter member list.

**Security:**
- Verifies that the caller is authenticated and is either a staff member or a registered member of the requested regional chapter, preventing IDOR leaks of member registries.
...
#### `getLocalMappers(proximityKm: number): Promise<{ success: boolean; data?: string[]; error?: string }>`
Retrieves a list of user IDs who are nearby active guardians, fuzzed and processed entirely server-side.
- **Security:** Processes geographic calculations using fuzzed database coordinates server-side to prevent exposing exact user/cat coordinate details to the client browser. Returns only matched user IDs.
...
### `GET /verify/[id]`
Verifies and displays spay/neuter certification records matching the computed cryptographic signature.
- **Path Parameter:** `id` (The proof of neuter UUID)
- **Query Parameters:**
  - `cat`: The cat ID
  - `vet`: The verifying clinic/vet ID
  - `date`: The verification timestamp
  - `sig`: HMAC-SHA256 signature generated by server using `NEUTER_HMAC_SECRET` (fails closed if missing, preventing default fallback bypasses)
